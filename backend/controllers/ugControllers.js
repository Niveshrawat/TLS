...
export const assignLeadToCaller = async (req, res) => {
  try {
    ...
    const lead = await Undergraduate.findByIdAndUpdate(
      leadId,
      { assignedTo: callerId },
      { new: true }
    );
...
export const updateLeadStatus = async (req, res) => {
  try {
    ...
    const lead = await Undergraduate.findById(leadId);
...
export const getLeadsForCaller = async (req, res) => {
  try {
    ...
    const leads = await Undergraduate.find({ assignedTo: callerId });
...
export const getUserAppliedColleges = async (req, res) => {
  try {
    const userId = req.user._id; // Assuming req.user._id is set by authentication middleware

    const colleges = await Undergraduate.find({ userId }).select('collegeName');
