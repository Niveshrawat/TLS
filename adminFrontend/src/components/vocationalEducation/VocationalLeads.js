...
import AddLeadForm from './CreateLead';
...
const CourseTable = () => {
  ...
  useEffect(() => {
    ...
    const fetchCallers = async () => {
      try {
        const response = await fetch('https://api.thelearnskills.com/api/v1/vcForm/vc-callers', {
          headers: {
    ...
    const fetchCourseLeads = async () => {
      try {
        const endpoint = role === 'caller'
          ? 'https://api.thelearnskills.com/api/v1/vcForm/caller-vc-leads'
          : 'https://api.thelearnskills.com/api/v1/vcForm/vc-forms';
  ...
  const handleAssign = async (leadId, callerId) => {
    const token = localStorage.getItem('token');
    try {
      const response = await fetch(`https://api.thelearnskills.com/api/v1/vcForm/assign-vc-lead/${leadId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleStatusChange = async (id, status) => {
    const token = localStorage.getItem('token');
    try {
      const response = await fetch(`https://api.thelearnskills.com/api/v1/vcForm/update-vc-lead-status/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  return (
    <div style={{marginTop:'5rem'}}>
      <Typography variant="h5" fontWeight="bold" gutterBottom>Vocational Leads</Typography>
          ...
          <TableBody>
            {courseLeads.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((courseLead, index) => (
              <TableRow key={courseLead._id}>
                ...
                <TableCell sx={{ textTransform: 'uppercase', padding: '8px' }}>{courseLead.name}</TableCell>
